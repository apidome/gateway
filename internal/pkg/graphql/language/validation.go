...
func checkFieldSelectionMergingInOpDef(
	selectionSet selectionSet,
	fragmentsPool map[string]*fragmentDefinition,
) {
	/* errMsg := "If multiple field selections with the same response " +
	"names are encountered during execution, the field and arguments " +
	"to execute and the resulting value should be unambiguous"; */
	panic("checkFieldSelectionMergingInOpDef not implemented")
}

func fieldsInSetCanMerge() bool {
	panic("fieldsInSetCanMerge not implemented")
}

func sameResponseShape(typeA, typeB _type) bool {
	_, isTypeANonNull := typeA.(*nonNullType)
	_, isTypeBNonNull := typeB.(*nonNullType)
	if isTypeANonNull != isTypeBNonNull {
		return false
	}

	panic("sameResponseShape not implementer")
}
...
func assertValueType(v value, t _type) bool {
	// TODO: Implement
	panic("assertValueType not implemented")
}
