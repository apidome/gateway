...
func IsValidHostname(hostname string) (bool, error) {
	...
	if len(hostname) > 255 {
		return false, errors.New("hostname is too long (more then 255 characters)")
	}
...
func IsValidIdnHostname(idnHostname string) (bool, error) {
	...
	for _, r := range idnHostname {
		...
		if disallowedIdnChars[s] {
			return false, errors.New(fmt.Sprintf("invalid hostname: contains illegal character %#U", r))
		}
	}
...
func IsValidIPv4(ipv4 string) (bool, error) {
	parsed := net.ParseIP(ipv4)
	hasDots := strings.Contains(ipv4, ".")
	if parsed == nil || !hasDots {
		return false, errors.New("invalid ipv4 address " + ipv4)
	}
...
func IsValidIPv6(ipv6 string) (bool, error) {
	...
	if parsed == nil || !hasColons {
		return false, errors.New("invalid ipv6 address " + ipv6)
	}
...
// RFC3986
// https://tools.ietf.org/html/rfc3986
func IsValidUriRef(uriRef string) (bool, error) {
	if _, err := url.Parse(uriRef); err != nil {
		return false, err
	}
	if strings.Contains(uriRef, "\\") {
		return false, errors.New("invalid uri-ref " + uriRef)
	}
	return true, nil
}
...
func IsValidIri(iri string) (bool, error) {
...
// according to [RFC3987].
// https://tools.ietf.org/html/rfc3987
func IsValidIriRef(iriRef string) (bool, error) {
...
func IsValidURITemplate(uriTemplate string) (bool, error) {
	...
	if strings.Contains(uriRef, "{") || strings.Contains(uriRef, "}") {
		return false, errors.New("invalid uri template " + uriTemplate)
	}
...
func IsValidJSONPointer(jsonPointer string) (bool, error) {
	...
	if jsonPointer[0] != '/' {
		return false, errors.New("non-empty references must begin with a '/' character: " + jsonPointer)
	}
	str := jsonPointer[1:]
...
func IsValidRelJSONPointer(relJSONPointer string) (bool, error) {
...
func IsValidRegex(regex string) (bool, error) {
